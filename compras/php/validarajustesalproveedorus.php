...
if (!empty($_POST)) {//si no esta vacío post entonces
    if(isset($_POST["rfc"]) && isset($_POST["estatus"])&& isset($_POST["tipo"])) { //si en post hay un rfc, un estus de proveedor y un tipo. entonces
        if($_POST["rfc"]!="" && $_POST["estatus"]!=""&& $_POST["tipo"]!=""){//si los campos por post no están vacíos entonces 
            ...
            $sql="UPDATE altaproveedor SET estatusp=\"$estatusp\",tipop=\"$tipop\" WHERE rfc=\"$rfc\"";//entonces creamos nuestra consulta con update y actualizamos los campos de la bd
            $query = $con->query($sql);
           if($query){
